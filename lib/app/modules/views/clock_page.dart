...
class _ClockPageState extends State<ClockPage> {
  ...
  Widget build(BuildContext context) {
    ...
    var formattedDate = DateFormat('EEE, d MMM', 'ru_RU').format(now);
...
class DigitalClockWidgetState extends State<DigitalClockWidget> {
  var formattedTime = DateFormat('HH:mm', 'ru_RU').format(DateTime.now());
  late Timer timer;
  ...
  void initState() {
    this.timer = Timer.periodic(Duration(seconds: 1), (timer) {
      ...
      if (perviousMinute != currentMinute)
        setState(() {
          formattedTime = DateFormat('HH:mm', 'ru_RU').format(DateTime.now());
        });
    });
    super.initState();
  }
